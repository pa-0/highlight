...
Custom::    Array, contains `Format` and `Style` elements to define custom styling
...
Hover          = Attibutes (LSP Hover elements)

Error          = Attibutes (LSP syntax errors)

ErrorMessage   = Attibutes (LSP error descriptions)
